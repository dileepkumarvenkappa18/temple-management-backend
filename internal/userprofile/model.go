...
type DevoteeProfile struct {
	...
	UserID   uint `gorm:"not null;index;uniqueIndex:ux_profile_user_entity,priority:1" json:"user_id"`   // one profile per user per entity
	EntityID uint `gorm:"not null;index;uniqueIndex:ux_profile_user_entity,priority:2" json:"entity_id"` // tenant/entity scope
	...
	SpecialInterestsOrNotes *string `gorm:"type:text" json:"special_interests_or_notes,omitempty"`
	...
	Children []*Child `gorm:"foreignKey:ProfileID;references:ID;constraint:OnUpdate:CASCADE,OnDelete:CASCADE" json:"children,omitempty"`
	EmergencyContacts []*EmergencyContact `gorm:"foreignKey:ProfileID;references:ID;constraint:OnUpdate:CASCADE,OnDelete:CASCADE" json:"emergency_contacts,omitempty"`

	// SECTION 6: Health, Sankalpa & Notes
	HealthNotes          *string `gorm:"type:text" json:"health_notes,omitempty"`
	AllergiesOrConditions *string `gorm:"type:text" json:"allergies_or_conditions,omitempty"`
	DietaryRestrictions  *string `gorm:"type:text" json:"dietary_restrictions,omitempty"`

	PersonalSankalpa *string `gorm:"type:text" json:"personal_sankalpa,omitempty"`
	AdditionalNotes  *string `gorm:"type:text" json:"additional_notes,omitempty"`

	// Profile Completion
	ProfileCompletionPercentage int `gorm:"not null;default:0" json:"profile_completion_percentage"`
...
func (DevoteeProfile) TableName() string {
	return "devotee_profiles"
}
...
type UserEntityMembership struct {
	ID       uint      `gorm:"primaryKey" json:"id"`
	UserID   uint      `gorm:"not null;index;uniqueIndex:ux_membership_user_entity,priority:1" json:"user_id"`
	EntityID uint      `gorm:"not null;index;uniqueIndex:ux_membership_user_entity,priority:2" json:"entity_id"`
	JoinedAt time.Time `gorm:"autoCreateTime" json:"joined_at"`
	Status   string    `gorm:"type:varchar(20);default:'active';index:idx_membership_status" json:"status"`
	CreatedAt time.Time `json:"created_at"`
}

func (UserEntityMembership) TableName() string {
	return "user_entity_memberships"
}
